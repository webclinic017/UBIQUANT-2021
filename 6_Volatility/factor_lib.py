...
def calculateFactor(merged_data, param_1=None, param_2=None, param_3=None, param_4=None):
    ...
    merged_data.loc[:,'Volatility'] = merged_data.groupby('Instrument')['Pct'].transform(lambda x: x.rolling(time_window).std())
    
    merged_data.loc[:,'Raw_factor'] = merged_data.groupby('Open time')['Volatility'].rank()
    print(merged_data[merged_data['Instrument']==6000].head(3))
    def RS(x):
        _buy = (x.Raw_factor<=(num_of_holding/2))          ## 做多波动率最低的
        _sell = (x.Raw_factor>(500-num_of_holding/2))      ## 做空波动率最大的
        x.loc[_buy, 'Factor'] = 1
