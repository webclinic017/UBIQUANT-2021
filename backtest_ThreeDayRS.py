...
def Alpha7(data):
    ...
    amount = pd.Series(np.array(data)[:, 7])              # 股票N天的volume数据
    mean_amount = amount.rolling(window=20).mean().iloc[-1]
    if mean_amount<amount.iloc[-1]:
        delta = close.diff(7).abs().iloc[-60:]
        delta_rank = delta.rank()
        return -1*delta_rank.iloc[-1]
    else:
        return -1
...
def each_task(_key, _array):
    ...
    factors.append(ThreeDayRS(_array))
...
while(my_sequence<total_days-5):
    ...
    if len(data[0]) >= 7:
        ...
        if len(factors_record) >= 10:
            ...
            # if len(factors_record) >= 10: factors_record.pop(0)
