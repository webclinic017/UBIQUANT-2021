...
def calculateFactor(merged_data, param_1=None, param_2=None, param_3=None, param_4=None):
    '''
    隔夜跳空因子
    param_1: time_window: 隔夜跳空的n日平均值
    '''
    time_window = param_1
    ...
    merged_data.loc[:, 'Close_shift'] = merged_data.groupby('Instrument')['Close'].shift(1)
    merged_data.loc[:,'OCDiff'] = np.abs(merged_data['Open']/merged_data['Close_shift']-1)
    merged_data.loc[:,'OCDiff_MA'] = merged_data.groupby('Instrument')['OCDiff'].transform(lambda x: x.rolling(time_window).mean())
    
    merged_data.loc[:,'Raw_factor'] = merged_data.groupby('Open time')['OCDiff_MA'].rank()
    print(merged_data[merged_data['Instrument']==6000].head(3))
    def RS(x):
        _buy = (x.Raw_factor<=(num_of_holding/2))          ## 做多跳空缺口最小的
        _sell = (x.Raw_factor>(500-num_of_holding/2))      ## 做空跳空缺口最大的
        x.loc[_buy, 'Factor'] = 1
