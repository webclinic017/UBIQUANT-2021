...
if __name__ == "__main__":
    ...
    if factor_name == 'EF':
        ...
        for i in range(T):
            ...
            # print(daily_data[['Mom', 'Size', 'VolumePct']])
            equal_factor = np.multiply(daily_data[['Mom', 'Size', 'VolumePct']], 
                                       factor_weight)
            equal_factor = equal_factor.sum(axis=1)
            # print(equal_factor)
            ...
            stock_pool = list(daily_data.loc[_buy, 'Instrument'])
            stock_sell = list(daily_data.loc[_sell, 'Instrument'])
            stock_pool.extend(stock_sell)
            # print(stock_pool)

            if i>60:
                ef_data = data[data['Date']<=sorted_dates[i]-2]
                ef_data = ef_data.pivot(index='Date', columns='Instrument', values='Pct')
                ef_data = ef_data[stock_pool]
                # print(ef_data)

                ef_data[stock_sell] *= -1
                ef_data.fillna(0, inplace=True)
                # print(ef_data)

                history_mean_return = ef_data.mean()
                history_cov = ef_data.corr()
                weight = max_sharpe_ratio(history_mean_return, history_cov, risk_free_rate=0.02)
                weight = weight['x']
                # print('ef weight:', weight)
            else:
                weight = len(stock_pool)*[1/N/2*10]

            # print(daily_data.loc[_buy, 'Pct'])
            each_return = list(daily_data.loc[_buy, 'Pct'])
            each_return.extend(list(-1*daily_data.loc[_sell, 'Pct']))
            daily_return = np.sum(np.multiply(each_return, weight))
            # print(daily_return)
