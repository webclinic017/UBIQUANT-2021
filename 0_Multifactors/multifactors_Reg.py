...
if __name__ == "__main__":
    ...
    if factor_name == 'Barra':
        for i in range(T):
            ...
            if i>60:
                X = daily_data[['Mom', 'Size', 'VolumePct']].values
                N = X.shape[0]
                country_factor = np.array(N * [[1]])
                X = np.hstack([country_factor, X])
                expected_ret = X @ factor_ret

                _rank = np.argsort(expected_ret)
                # print(_rank)
            ...
            if (i%10 == 0) and (i//10 >= 6):
                ...
                factor_ret = []
                while index>i-60:
                    one_month = data[(data['Date']<=sorted_dates[index]) & (data['Date']>=sorted_dates[index]-20)]
                    one_month = one_month.fillna(0)
                    X = one_month[['Mom', 'Size', 'VolumePct']].values
                    N = X.shape[0]
                    country_factor = np.array(N * [[1]])
                    X = np.hstack([country_factor, X])
                    Y = one_month['Pct'].values
                    print(X)
                    print(Y)
                    factor_ret_t = np.linalg.inv(X.T @ X) @ X.T @ Y
                    # specific_ret = Y - X @ factor_ret
                    factor_ret.append(factor_ret_t)
                    index -= 20
                factor_ret = np.mean(factor_ret, axis=0)
                print(factor_ret)
    else:
