...
四因子 + Alpha101，去除相关性>0.3的，剩余6个因子
用MFM库回归试一试（Barra模型）
...
while(my_sequence<total_days-5):
    ...
    if len(data[0]) >= 20:
        ...
        if len(factors_weight) == 0:
            ...
            final_factors = np.hstack(([[1]]*num_of_stock, all_factors)) @ factors_weight
        ...
        if factors_record_len >= 20:
            ...
            column_name.extend(strategy_list)
            data_mfm = pd.DataFrame(columns=column_name)
            
            # 其实只做了8次回归，取平均值
            factors_weight = []
            for i in range(factors_record_len-2):
                factors_tmp = factors_record[-(i+3)]                       # N * K
                factors_tmp = np.hstack((np.array(pct_record[-(i+1)]).reshape(-1,1), factors_tmp))   # t+2 收益率，N * 1
                factors_tmp = np.hstack(([[1]]*num_of_stock, factors_tmp))   # 股票市值，# 只能假设市值相等                        
                factors_tmp = np.hstack((np.arange(num_of_stock).reshape(-1,1), factors_tmp))   # 股票id
                factors_tmp = np.hstack(([[i]]*num_of_stock, factors_tmp))                      # 日期
                data_tmp = pd.DataFrame(factors_tmp, columns=column_name)
                # print(data_tmp)
                data_mfm = data_mfm.append(data_tmp, ignore_index=True)
            
            # print(data_mfm)
            model = MFM(data_mfm, 0, 8)                                       # 数据，行业因子数，风格因子数
            factors_weight, specific_ret, R2 = model.reg_by_time()
            # print(factors_weight)
                
            factors_weight = factors_weight.mean().values      # 按列平均
            ...
            if factors_record_len >= 20: factors_record.pop(0)
