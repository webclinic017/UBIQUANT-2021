...
def calculateFactor(merged_data, param_1=None, param_2=None, param_3=None, param_4=None):
    ...
    merged_data.loc[:,'6dayRS'] = merged_data.groupby('Instrument')['Close'].pct_change(time_window)
    merged_data.loc[:,'Raw_factor'] = merged_data.groupby('Open time')['6dayRS'].rank()
    print(merged_data[merged_data['Instrument']==6000].head(3))
    def RS(x):
        _sell = (x.Raw_factor>(500-num_of_holding/2))  ## 卖出涨得最多的
        _buy = (x.Raw_factor<=(num_of_holding/5))      ## 买入跌得最厉害的
        x.loc[_buy, 'Factor'] = 1
