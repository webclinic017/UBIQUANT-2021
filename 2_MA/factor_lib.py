...
def calculateFactor(merged_data, param_1=None, param_2=None, param_3=None, param_4=None):
    '''
    双均线因子
    param_1: time_window: 代表n日收益率
    '''
    short_t = param_1
    long_t = param_2
    num_of_holding = param_3
    ...
    merged_data.loc[:, 'CumMax'] = merged_data.groupby('Instrument')['Close'].cummax()
    merged_data.loc[:, 'CumMin'] = merged_data.groupby('Instrument')['Close'].cummin()
    merged_data.loc[:, 'MADelta'] = (merged_data['SMA']-merged_data['LMA'])/(merged_data['CumMax']-merged_data['CumMin'])

    merged_data.loc[:,'Raw_factor'] = merged_data.groupby('Open time')['MADelta'].rank()
    print(merged_data[merged_data['Instrument']==6000].head(3))
    def RS(x):
        _buy = (x.Raw_factor>(500-num_of_holding/2)) & (x.MADelta>0)       ## 追涨
        _sell = (x.Raw_factor<=(num_of_holding/2)) & (x.MADelta<0)         ## 杀跌
        x.loc[_buy, 'Factor'] = 1
